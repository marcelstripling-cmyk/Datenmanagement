# dashboard.py
# Empfohlene Installation (lokal oder in requirements.txt):
...
st.markdown("Lade deine CSV hoch und speichere interessante Ansichten innerhalb des Dashboards (nur während der Session).")
...
if uploaded_file is not None and st.session_state.df is None:
    try:
        ...
        possible_years = [c for c in df.columns if any(k in str(c).lower() for k in ["jahr", "year", "date", "zeit", "monat"])]
        possible_prices = [c for c in num_cols if any(k in str(c).lower() for k in ["preis", "price", "kosten", "eur", "€", "wert"])]
        ...
        st.caption("Gefundene numerische Spalten: " + ", ".join(num_cols) if num_cols else "Keine numerischen Spalten gefunden!")
        
    except Exception as e:
        st.error(f"Fehler beim Einlesen der Datei:\n{str(e)}")
...
if st.session_state.df is not None:
    ...
    with col1:
        x_col = st.selectbox(
            ...
            index=df.columns.get_loc(st.session_state.default_year) 
                  if st.session_state.get("default_year") in df.columns 
                  else 0,
            key="x_col"
        )
    ...
    with col2:
        ...
        if not numeric_cols:
            ...
            st.info(
                "Die Y-Achse (Preis) benötigt mindestens eine Spalte mit Zahlen.\n\n"
                "Mögliche Ursachen:\n"
                "• Preise enthalten €, Kommas oder Tausenderpunkte\n"
                "• Die Spalte ist als Text formatiert\n"
                "• Es gibt keine Preisspalte in der CSV"
            )
            y_col = st.selectbox(
                "Y-Achse (Preis) – Fallback (alle Spalten)",
                options=df.columns.tolist(),
                index=0,
                key="y_col_fallback"
            )
    ...
    # Früher Abbruch, wenn keine vernünftige Y-Spalte möglich ist
    if not numeric_cols and y_col is None:
        st.stop()
    ...
    # ── Die drei Analysen ────────────────────────────────────────
    analyses = [
        {"name": "1. Preisspanne teuer ↔ günstig", "desc": "Min und Max pro Zeitraum", "type": "range"},
        {"name": "2. Median vs. Durchschnitt", "desc": "Vergleich pro Zeitraum", "type": "median_mean"},
        {"name": "3. Positive & negative Ausreißer", "desc": "IQR-Methode (1.5×IQR)", "type": "outliers"}
    ]
    ...
    for idx, analysis in enumerate(analyses):
        with cols[idx]:
            ...
            if analysis["type"] == "range":
                ...
                fig.update_layout(title="Preisspanne pro Zeitraum", xaxis_title=x_col, yaxis_title=y_col, hovermode="x unified")
                ...
                if len(outliers) > 0:
                    fig = px.scatter(
                        ...
                        title="Ausreißer pro Zeitraum",
                        color_discrete_map={"positiver Ausreißer": "#d32f2f", "negativer Ausreißer": "#1976d2"}
                    )
                else:
                    fig = go.Figure()
    ...
    if st.session_state.saved_views:
        st.write(f"{len(st.session_state.saved_views)} gespeicherte Ansichten vorhanden")
        ...
        st.info("Noch keine Ansichten gespeichert. → Oben auf »Diese Ansicht speichern« klicken")

else:
    st.info("Bitte zuerst eine CSV-Datei hochladen.")

st.caption("Dashboard – nur Session-interne Speicherung • 2026")
